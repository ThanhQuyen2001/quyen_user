  ...
  scaledSize: { width: 50, height: 50 },
    ...
    <div>
      <b-modal id="bv-modal-network" size="sm" hide-footer fade hide-header centered>
        <div class="div">
          <img class="img-cover rounded" style="height: 165px" :src="$addPrefixImage(makerSelected.avatar)" alt="" />
        </div>
        ...
        <div v-show="isOpenFullModel">
          ...
          <div class="d-flex py-2">
            ...
            <span @click="$router.push({ path: `/${makerSelected.url}/home` })"
              class="ml-1 font-size-12 text-default font-weight-bold pointer">
              Khám phá ngay
            </span>
          </div>
        </div>
          ...
          <i @click="isOpenFullModel = !isOpenFullModel" :class="{
            ...
            'bx bx-chevron-up p-1 text-white pointer':
              isOpenFullModel == true,
                    ...
                    background-color: #44b97c;font-size: 18px;border-radius: 50%;"></i>
        </div>
      </b-modal>
    </div>
  </div>
  ...
  data() {
    return {
      ...
      makerSelected: {
        ...
        phone: '(+84) 0543553414',
        gmail: 'phonghoa@thuathienhue.gov.vn',
